...
public class Unit {
    ...
    public Party party;
    public float LVL, MP, MAX_HP = 20, MAX_MP,BASE_MAG=10, MAG =BASE_MAG, BASE_STR = 30, STR=BASE_STR,  BASE_DEF=20, DEF=20, BASE_SPD=0, SPD=BASE_SPD;
    ...
    public void takeDamage(float dmg, @Nullable Element element, boolean crit){
        ...
        if(crit){
            updateIndicator(new CritIndicator((int) dmg));
        }
        if(element!=null&&this.affinities.get(element).equals(Affinity.WEAK)){
            updateIndicator(new WeakIndicator((int) dmg));
        }
        else if(element!=null&&this.affinities.get(element).equals(Affinity.STRONG)){
            updateIndicator(new ResistIndicator((int) dmg));
        }
        else {
            updateIndicator(new DmgIndicator((int) dmg));
        }
