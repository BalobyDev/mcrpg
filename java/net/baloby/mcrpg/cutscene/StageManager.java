...
public class StageManager extends JsonReloadListener {
    private static Gson GSON = new GsonBuilder().create();

    protected Map<ResourceLocation, Stage> data;

    public Stage getData(ResourceLocation id){
    ...
    protected void apply(Map<ResourceLocation, JsonElement> p_212853_1_, IResourceManager p_212853_2_, IProfiler p_212853_3_) {
        Map<ResourceLocation, Stage> newMap = new HashMap<>();

        for (Map.Entry<ResourceLocation, JsonElement> entry : p_212853_1_.entrySet()){
            ResourceLocation key = entry.getKey();
            JsonElement element = entry.getValue();

            Stage.CODEC.decode(JsonOps.INSTANCE, element)
                    .get()
