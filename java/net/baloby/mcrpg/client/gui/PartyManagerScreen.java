...
public class PartyManagerScreen extends Screen {
    ...
    public boolean mouseClicked(double x, double y, int key){
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 4; j++) {
                if(((x>relX+(j*60)+9)&&(x<relX+(j*60)+63))
                &&((y>relY+8+(i*95))&&(y<relY+8+(i*95)+BOX_SIZE))){
                    if(held!=null&&!profiles.containsKey(k)){
                        Integer heldKey = null;
                        for(Map.Entry<Integer, Profile> entry: profiles.entrySet()){
                            if(entry.getValue().equals(held)){
                                heldKey = entry.getKey();
                            }}

                        profiles.put(k,held);
                        profiles.remove(heldKey);
                        this.held = null;
                        this.player.getCapability(PlayerCapabilityProvider.CHAR_CAP).resolve().get().getPartyMembers().remove(heldKey+"");
                        this.player.getCapability(PlayerCapabilityProvider.CHAR_CAP).resolve().get().setPartyMembers(profiles);
    ...
    private void renderProfile(Profile profile, MatrixStack matrixStack, int x, int y){
        ...
        if(profile==this.hovered){
            this.minecraft.getTextureManager().bind(GUI);
            this.blit(matrixStack,x-28,y,55,200,54,54);
        }
